...
const getDataId = async () => {
  try {
    const response = await Api.get(`gurubk/catatan/prestasi/${id}/data/${id2}`);
...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.put(
      `gurubk/catatan/prestasi/${id}/data/${id2}`,
      dataStore
    );
...
<template>
  <div class="pt-4 px-10 md:flex justify-between" v-if="dataSiswa">
    <div>
      <span
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content> Kasus <BreadCrumbSpace /> Tambah </template>
      </BreadCrumb>
    </div>
  </div>
