...
const doEditData = async (id) => {
  router.push({ name: "AdminBkEdit", params: { id } });
};

const doDeleteData = async (id, index) => {
  if (confirm("Apakah anda yakin menghapus data ini?")) {
    try {
      const response = await Api.delete(`gurubk/bk/${id}`);
...
<template>
  <div class="pt-4 px-10 md:flex justify-between">
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content> Guru BK <BreadCrumbSpace /> Index </template>
      </BreadCrumb>
    </div>
  </div>
  ...
  <div class="pt-4 px-10 md:flex justify-between">
    <div>
      <router-link :to="{ name: 'AdminBkTambah' }">
        <button
